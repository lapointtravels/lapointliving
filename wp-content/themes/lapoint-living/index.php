<?php get_header(); ?>

	<?php // get_template_part( 'content', get_post_format() ); ?>

	<div class="o-section o-section--hero">
	  <div class="o-container o-container--large">
	    <div class="c-navigation">
	      <button class="c-navigation__menu-trigger"><img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-hamburger.svg" alt="Icon hamburger" /></button>
	      <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/lapoint-living-logo@2x.png" class="c-navigation__logo" width="105" alt="Lapoint living logo@2x" />
	    </div>
	  </div>
	  <div class="o-container">
	    <div class="c-hero">
	      <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-villa.svg" class="c-hero__icon" alt="Icon villa" />
	      <h1 class="c-hero__title">
	        Garden Villa
	        <span class="c-hero__subtitle">Ahangama, Sri Lanka</span>
	      </h1>
	      <p class="c-hero__lead">
	        This villa is situated near the beach and optimal surf conditions. Relax and enjoy a cold drink in one of our shaded hang out lounges in our tropical garden.
	      </p>
	      <a href="#booking-form" class="c-button c-button--primary c-hero__cta">Book now</a>
	    </div>
	  </div>
	</div>

	<div class="o-section o-section--padded">
	  <div class="o-container">
	    <h2 class="u-section-header">The villa</h2>
	    <p class="u-text-center">Camp Ahangama offers hang-out in garden areas, on balconies or in sunbeds. You can also enjoy our private gardens for sunbathing, activities and fun. The chill vibe at this camp is perfect for some time off after a long surf session.</p>
	  </div>
	</div>


	<div class="o-section">
	  <div class="c-media-grid">
	    <div class="c-media-grid__cell c-media-grid__cell--media">
	      <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/garden-villa-1.jpg" alt="Garden villa 1" />
	    </div>
	    <div class="c-media-grid__cell">
	      <h3>Services and facilities</h3>
	      <ul class="o-flow-grid u-text-center">
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-pool.svg" alt="Icon pool" />
	          <p class="u-text-sans-serif u-text-small">Pool</p>
	        </li>
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-wifi.svg" alt="Icon wifi" />
	          <p class="u-text-sans-serif u-text-small">Wifi</p>
	        </li>
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-cleaning.svg" alt="Icon cleaning" />
	          <p class="u-text-sans-serif u-text-small">Daily cleaning</p>
	        </li>
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-yoga.svg" alt="Icon yoga" />
	          <p class="u-text-sans-serif u-text-small">Yoga area</p>
	        </li>
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-bar.svg" alt="Icon bar" />
	          <p class="u-text-sans-serif u-text-small">Bar</p>
	        </li>
	        <li>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-volleyball.svg" alt="Icon volleyball" />
	          <p class="u-text-sans-serif u-text-small">Volleyball court</p>
	        </li>
	      </ul>
	    </div>
	    <div class="c-media-grid__cell c-media-grid__cell--media">
	      <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/garden-villa-2.jpg" alt="Garden villa 2" />
	    </div>
	    <div class="c-media-grid__cell">
	      <h3>Camp facts</h3>
	      <ul class="o-flow-grid u-text-center">
	        <li>
	          <span class="u-text-huge u-text-sans-serif">220</span>
	          <p class="u-text-sans-serif u-text-small">Sq. m interior</p>
	        </li>
	        <li>
	          <span class="u-text-huge u-text-sans-serif">175</span>
	          <p class="u-text-sans-serif u-text-small">Sq. m exterior</p>
	        </li>
	        <li>
	          <span class="u-text-huge u-text-sans-serif">4</span>
	          <p class="u-text-sans-serif u-text-small">Dorm rooms</p>
	        </li>
	        <li>
	          <span class="u-text-huge u-text-sans-serif">4</span>
	          <p class="u-text-sans-serif u-text-small">Double rooms</p>
	        </li>
	        <li>
	          <span class="u-text-huge u-text-sans-serif">30</span>
	          <p class="u-text-sans-serif u-text-small">Beds total</p>
	        </li>
	      </ul>
	    </div>
	  </div>
	</div>

	<div class="o-section o-section--book-now u-text-center">
	  <a href="#booking-form" class="c-button c-button--primary">Book now</a>
	</div>

	<div class="o-section o-section--padded">
	  <div class="o-container o-container--large">
	    <h2 class="u-section-header">The rooms</h2>
	    <div class="o-room-grid">
	      <div class="o-room-grid__cell">
	        <ul class="c-accordion">
	          <li class="c-accordion__item is-active">
	            <button class="c-accordion__title">Dorm room</button>
	            <div class="c-accordion__body">
	              Sharing room is a great way to meet new people. Camp Ahangama has dorms that can accommodate four people each. This is the perfect option for those who are travelling alone, with a group of friends or with the family.
	              <a href="#booking-form" class="c-button c-button--small c-button--block">Book dorm room</a>
	            </div>
	          </li>
	          <li class="c-accordion__item">
	            <button class="c-accordion__title">Double room</button>
	            <div class="c-accordion__body">
	              Sharing room is a great way to meet new people. Camp Ahangama has dorms that can accommodate four people each. This is the perfect option for those who are travelling alone, with a group of friends or with the family.
	              <a href="#booking-form" class="c-button c-button--small c-button--block">Book double room</a>
	            </div>
	          </li>
	          <li class="c-accordion__item">
	            <button class="c-accordion__title">Single room</button>
	            <div class="c-accordion__body">
	              Sharing room is a great way to meet new people. Camp Ahangama has dorms that can accommodate four people each. This is the perfect option for those who are travelling alone, with a group of friends or with the family.
	              <a href="#booking-form" class="c-button c-button--small c-button--block">Book single room</a>
	            </div>
	          </li>
	        </ul>
	      </div>
	      <div class="o-room-grid__cell">
	        <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/dorm-room.jpg" alt="Dorm room" />
	      </div>
	    </div>
	  </div>
	</div>

	<div class="o-section o-section--padded">
	  <div class="o-container">
	    <h2 class="u-section-header">The area</h2>
	    <h3>Sri Lanka</h3>
	    <p class="u-margin-top">Sri Lanka is an island southeast of India and is a tropical paradise with world class waves. Here you’ll get surf courses in idyllic surroundings with beautiful nature, yoga, chill vibe and great waves. ‘Lanka is home to mile long beaches, a tropical climate and many exciting places to visit. Together with a great cuisine and amazing surf Sri Lanka offers an ocean of possibilities for all types of explorers and surfers.</p>
	  </div>
	  <div class="c-band">
	    <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/ahangama.jpg" alt="Ahangama" />
	  </div>
	  <div class="o-container">
	    <h3>Ahangama</h3>
	    <p class="u-margin-top">Sri Lanka is an island southeast of India and is a tropical paradise with world class waves. Here you’ll get surf courses in idyllic surroundings with beautiful nature, yoga, chill vibe and great waves. ‘Lanka is home to mile long beaches, a tropical climate and many exciting places to visit. Together with a great cuisine and amazing surf Sri Lanka offers an ocean of possibilities for all types of explorers and surfers.</p>
	  </div>
	</div>

	<div class="o-section o-section--padded">
	  <div class="o-container o-container--large">
	    <h2 class="u-section-header">Activities</h2>
	    <ul class="c-banner-list">
	      <li class="c-banner-list__item">
	        <div>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/activities-surfing.jpg" class="c-banner-list__item-bg" alt="Activities surfing" />
	          <h3>Surfing</h3>
	          <p>We want to share the stoke with you when you catch your first wave or your first barrel.</p>
	        </div>
	        <ul class="c-banner-list__subsection">
	          <li class="c-banner-list__subitem">
	            <h4>Level 1</h4>
	            <p>This course is for you that have never surfed before, or that haven’t surfed in a while and want to go through the basics one more time.</p>
	            <a href="#" class="c-button c-button--hollow c-button--block">Read more</a>
	            <a href="#" class="c-button c-button--inverted c-button--block">Book now</a>
	          </li>
	          <li class="c-banner-list__subitem">
	            <h4>Level 2</h4>
	            <p>This level is designed for those of you that have passed our level 1 or can catch and ride a white water wave on your own.</p>
	            <a href="#" class="c-button c-button--hollow c-button--block">Read more</a>
	            <a href="#" class="c-button c-button--inverted c-button--block">Book now</a>
	          </li>
	          <li class="c-banner-list__subitem">
	            <h4>Level 3</h4>
	            <p>This level is designed for those of you that have passed our level 2/can catch waist high green waves on your own and surf frontside and backside.</p>
	            <a href="#" class="c-button c-button--hollow c-button--block">Read more</a>
	            <a href="#" class="c-button c-button--inverted c-button--block">Book now</a>
	          </li>
	        </ul>
	      </li>
	      <li class="c-banner-list__item">
	        <div>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/activities-supy.jpg" class="c-banner-list__item-bg" alt="Activities supy" />
	          <h3>Stand up paddle yoga</h3>
	          <p>Experience a new element of core strength, intense focus combined with sun, refreshing water and beautiful nature.</p>
	          <a href="#" class="c-button c-button--hollow">Read more</a>
	        </div>
	      </li>
	      <li class="c-banner-list__item">
	        <div>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/activities-safari.jpg" class="c-banner-list__item-bg" alt="Activities safari" />
	          <h3>Safari</h3>
	          <p>Lorem ipsum dolor sit amet.</p>
	          <a href="#" class="c-button c-button--hollow">Read more</a>
	        </div>
	      </li>
	      <li class="c-banner-list__item">
	        <div>
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/activities-plantage.jpg" class="c-banner-list__item-bg" alt="Activities plantage" />
	          <h3>Tea planatations</h3>
	          <p>Lorem ipsum dolor sit amet.</p>
	          <a href="#" class="c-button c-button--hollow">Read more</a>
	        </div>
	      </li>
	    </ul>
	  </div>
	</div>

	<div class="o-section o-section--padded o-section--inverted" id="booking-form">
	  <div class="o-container o-container--large">
	    <h2 class="u-section-header--inverted">Book your stay</h2>
	    <div class="c-flex-block c-flex-block--white">
	      <div class="c-flex-block__cell">
	        <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-villa--red.svg" alt="Icon villa  red" />
	        <h3 class="u-margin-top">Garden Villa</h3>
	        <p class="u-text-sans-serif u-text-small">Ahangama, Sri Lanka</p>
	      </div>
	      <div class="c-flex-block__cell">
	        <div class="c-form-group">
	          <label for="duration">Duration:</label>
	          <select class="" name="duration">
	            <option value="">1 week</option>
	            <option value="">2 weeks</option>
	            <option value="">3 weeks</option>
	          </select>
	        </div>
	      </div>
	      <div class="c-flex-block__cell">
	        <div class="c-form-group">
	          <label for="month">Month:</label>
	          <select class="" name="month">
	            <option value="">January 2018</option>
	            <option value="">February 2018</option>
	            <option value="">March 2018</option>
	            <option value="">April 2018</option>
	            <option value="">May 2018</option>
	            <option value="">June 2018</option>
	            <option value="">July 2018</option>
	            <option value="">August 2018</option>
	            <option value="">September 2018</option>
	          </select>
	        </div>
	      </div>
	    </div>
	    <ul class="c-booking-list">
	      <li class="c-booking-list__row c-booking-list__row--header">January 2018</li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Jan 15 – Jan 22</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (4 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Jan 22 – Jan 29</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (full)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted" disabled>Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Jan 29 – Feb 5</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (2 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row c-booking-list__row--header">February 2018</li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Feb 5 – Feb 12</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (2 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Feb 12 – Feb 19</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (4 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Feb 19 – Feb 26</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (7 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	      <li class="c-booking-list__row">
	        <div class="c-booking-list__cell">
	          <img src="<?php echo get_bloginfo( 'template_directory' );?>/assets/icon-calendar.svg" class="c-booking-list__icon" alt="Icon calendar" />
	          <span class="c-booking-list__date">Feb 26 – March 2</span>
	        </div>
	        <div class="c-booking-list__cell">
	          <div class="c-form-group c-form-group--inline">
	            <label>Surf level:</label>
	            <select>
	              <option selected>B&amp;B only (6 left)</option>
	              <option>Level 1 (1 left)</option>
	              <option>Level 2 (4 left)</option>
	              <option>Level 3 (2 left)</option>
	            </select>
	          </div>
	          <button class="c-button c-button--inverted">Book</button>
	        </div>
	      </li>
	    </ul>
	    <div class="c-flex-block">
	      <div class="c-flex-block__cell">
	        <a href="#" class="c-button c-button--hollow c-button--left-pointer">Earlier dates</a>
	      </div>
	      <div class="c-flex-block__cell">
	        <a href="#" class="c-button c-button--hollow c-button--right-pointer">Later dates</a>
	      </div>
	    </div>
	  </div>
	</div>

<?php get_footer(); ?>
